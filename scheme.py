...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals): raise SchemeError('non-matching arguments')
        else:
            #print('formals', formals, 'vals', vals)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            if self.use_env == True:  python_args.append(env)
            return self.fn(*python_args)

        except TypeError:
            raise SchemeError('Invalid type error')
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    #seperate into Pair lists of formals and values
    formals = nil
    vals = nil
    
    while bindings is not nil:
        # check that all bindings are valid
...
def do_define_macro(expressions, env):
    ...
    formals = target.second
    body = expressions.second
    env.define(symb, MacroProcedure(formals, body, env))
    return symb
